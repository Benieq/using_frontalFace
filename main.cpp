...
void detectAndDraw(
    ...
    cv::Size s(cvRound(img.cols/scale),
               cvRound(img.rows/scale));
    
    cv::Mat gray(img.size(), CV_8UC1);
    cv::Mat small_img(s, CV_8UC1);
...
//        cv::Size(30, 30)
                                 );          // throw away detections smaller than this
...
int main(int argc, char** argv) {
    ...
    string image_file_name = "/Users/wuxilei/Desktop/test_pic/faces.png";
    cv::Mat img = cv::imread(image_file_name);
//    cv::Mat img = cv::imread(image_file_name, CV_LOAD_IMAGE_COLOR);


    string cascade_file_name = "/Users/wuxilei/Desktop/haarcascade_frontalface_alt2.xml";
    ...
    cv::waitKey(0);
    cv::imwrite("/Users/wuxilei/Desktop/outimg.png", img);
    return 0;
}
